...
CREATE FUNCTION OrdenesClientesPorCiudad(ciudad_nombre VARCHAR(50))
RETURNS INT
READS SQL DATA
BEGIN
...
DROP VIEW IF EXISTS VistaDeliveriesxCiudad;
...
-- VISTA 3: Vista que muestra las facturas en las cuales el importe es menor a 1000 y el periodo es el primer semestre del 2023.
...
-- VISTA 5: Vista que muestra los deliverys que se hicieron para cada Ciudad

CREATE VIEW VistaDeliveriesxCiudad AS
SELECT c.Nombre AS Ciudad, COUNT(*) AS TotalEntregas
FROM DELIVERY d
JOIN CIUDADES c ON d.Id_Ciudad = c.Id_Ciudad
GROUP BY c.Nombre;

-- STORED PROCEDURES
-- STORED PROCEDURE 1: SE PIDEN DOS PARAMETROS, EL PRIMERO ES PARA ORDENAR LA TABLA 'ORDEN' SEGUN EL, Y EL SEGUNDO PARA INDICAR SI ES ASCENDENTE O DESCENDENTE
...
-- STORED PROCEDURE 2 - SE DISEÑA UN SP PARA AGREGAR UN CLIENTE A TABLA CLIENTES.
