...
CREATE TABLE IF NOT EXISTS log_facturacion LIKE facturacion;
CREATE TABLE IF NOT EXISTS log_orden LIKE orden;
ALTER TABLE log_facturacion
DROP COLUMN Id_Orden,
DROP COLUMN Id_Producto,
DROP COLUMN Id_Cliente,
DROP COLUMN Id_Delivery,
DROP COLUMN Mesa;

ALTER TABLE log_facturacion
    ADD COLUMN usuario CHAR(50),
    ADD COLUMN fecha DATE,
    ADD COLUMN hora TIME;

ALTER TABLE log_orden
DROP COLUMN Importe,
DROP COLUMN Id_Mpago,
DROP COLUMN Fecha,
DROP COLUMN Mesa,
...
-- STORE PROCEDURES
-- STORE PROCEDURE 1: SE PIDEN DOS PARAMETROS, EL PRIMERO ES PARA ORDENAR LA TABLA 'ORDEN' SEGUN EL, Y EL SEGUNDO PARA INDICAR SI ES ASCENDENTE O DESCENDENTE
...
-- STORE PROCEDURE 2 - SE DISEÑA UN SP PARA AGREGAR UN CLIENTE A TABLA CLIENTES.
...
-- TRIGGERS
-- trigger  1: Se realiza la creacion de un trigger para que una vez que se ingrese un registro en la tabla orden, se realice una copia de seguridad en la tabla de auditoria log_orden.
-- en la misma se detalla el usuario, la fecha y hora en la cual se grabo el registro

DROP TRIGGER IF EXISTS tr_add_new_orden;
DELIMITER $$
-- Se crea el trigger en la tabla orden.
CREATE TRIGGER tr_add_new_orden AFTER INSERT ON orden
FOR EACH ROW
BEGIN
...
-- TRIGGER 2:Se crea un trigger para que ante un borrado de registro en la tabla facturacion, se resguarde el Id_Factura, el importe y los datos de usuario, fecha y hora en la tabla log_facturacion.
DROP TRIGGER IF EXISTS tr_del_facturacion;

DELIMITER $$

-- Se crea el trigger en la tabla orden

CREATE TRIGGER tr_del_facturacion BEFORE DELETE ON facturacion
FOR EACH ROW
BEGIN
...
-- Se realiza la creacion de un SP para Agregar/Eliminar registros de la tabla ORDEN. En caso que tenga registros, eliminara de a uno. Para el caso que este vacia, agregara 5 registros.
...
-- Sentencia Sublenguaje TCL 2:
-- Se realiza la insercion de los primeros 4 registros en la tabla 'Ciudades' y se setea un savepoint lote_1 para establecer un punto de restauracion.
START TRANSACTION;

INSERT INTO Ciudades (Nombre) VALUES ('Naranjos');
INSERT INTO Ciudades (Nombre) VALUES ('Rosario');
INSERT INTO Ciudades (Nombre) VALUES ('Tigre');
INSERT INTO Ciudades (Nombre) VALUES ('Tesei');

savepoint lote_1;
-- Se realiza la insercion de los ultimos 4 registros en la tabla 'Ciudades' y se setea un savepoint lote_2 para establecer un punto de restauracion.
INSERT INTO Ciudades (Nombre) VALUES ('Grandburg');
INSERT INTO Ciudades (Nombre) VALUES ('Pilar');
INSERT INTO Ciudades (Nombre) VALUES ('Belgrano');
INSERT INTO Ciudades (Nombre) VALUES ('San_Cristobal');

savepoint lote_2;
-- eliminacion del primer savepoint (lote_1)
release savepoint lote_1;

-- Confirmacion o rollback de transaccion.
