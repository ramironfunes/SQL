...
declare @CodPractEscClinico int;
		...
		--set @buscarNombre = 'simica';

		--Pk de la tabla PrestItem:
		set @PrestItemPK = 54552;

		--PK de la tabla VIADUCT_MAP_EC_LIS 
		set @CodPractEscClinico = 2666;

-- Tabla de mapeo entre Escritorio Clinico y Lis (Laboratory)
select top 10 * from VIADUCT_MAP_EC_LIS
where COD_PRACTICA_ESC_CLIN = @CodPractEscClinico
	--where DESC_PRUEBA_EGES like '%'+ @buscarNombre +'%'

-- Tabla de mapeo entre xHis y Lis 
select top 10 * from VIADUCT_MAP_HIS_LIS
