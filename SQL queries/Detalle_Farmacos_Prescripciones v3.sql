SELECT hc.nhc, 
	   ...
	   case when clientes.apellido2 is NULL then ' ' ELSE clientes.apellido2 END as 'Apellido2',
      ...
      -- ingresos.cod_cama AS 'Cama Ingreso', --es la cama actual del paciente, entiendo que esa cama no va a ser necesaria.
       ...
       art_mc_nom AS Prescripcion,
       far_det_pac_car.far_detpaccar_cant AS cantidad,
	   far_detalles_ord.fdo_fecha_crea + far_detalles_ord.fdo_hora_crea as Fecha_Prescripcion,
	   fpersona.nombre + ' ' + fpersona.apellido1 as Prescriptor,
	   ...
	   far_detmed_dia + far_detmed_hora AS 'Fecha Prevista Ingesta',
       ingesta.ingta_realif+ingesta.ingta_realih as 'Fecha Ingesta',
       ...
       case when ingesta.ingta_urg_sn = 1 then 'Urgente' else 'Ordinaria' end as 'Ingesta Urgente/Ordinaria',
	   pauta.pauta_den as Pauta
...
FROM far_det_pac_car
	 ...
	 -- JV
	 INNER JOIN far_detalles_ord ON far_detalles_ord.linea_orden_pk = far_det_pac_car.linea_orden_pk
	 INNER JOIN fpersona ON fpersona.codigo_personal = far_detalles_ord.fdo_crea_usu_pk
...
WHERE (far_det_pac_car.fdpc_estado_hp IN (4,11) OR (far_det_pac_car.fdpc_estado_hp = 10 AND far_det_pac_car.fdpc_ingta_pk IS NOT NULL))
      ...
      AND far_detmed_dia >= getdate() -31  --Fecha Desde
      --AND far_detmed_dia <= '21/11/2021'  --Fecha Hasta
      ...
      --and hc.nhc = '121251491'            --NHC


ORDER BY nhc, cod_bar_mc_pr, far_detmed_dia, far_detmed_hora